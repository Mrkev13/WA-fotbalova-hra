...
async function firePlayer(playerId, name) {
  if (!confirm('Opravdu propustit hráče ' + (name || '') + '? Dostaneš zpět 50 % z jeho tržní hodnoty.')) return;
  try {
    const res = await apiFetch('POST', '/fire_player', { playerId });
    if (currentUser && res.newMoney !== undefined) {
      currentUser.money = res.newMoney;
      saveUser();
      syncTopbar();
    }
    alert(res.message);
